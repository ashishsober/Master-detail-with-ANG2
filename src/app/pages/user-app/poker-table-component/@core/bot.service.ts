...
export class BotService {
    ...
    get_preflop_bet(players, current_bettor_index) {
        this.setup(players, current_bettor_index);

        if (this.genericMethods.HUMAN_WINS_AGAIN > 2 && (this.HCONF > 60 || this.RANKA == this.RANKB || this.RANKA > 13 || this.RANKB > 13)) {
    ...
    //ETC.
    setup(players, current_bettor_index) {
        this.P = players[current_bettor_index];
        this.CALL = this.genericMethods.current_bet - this.P.subtotal_bet;
        this.RANKA = this.handService.get_rank(this.P.carda);
        this.RANKB = this.handService.get_rank(this.P.cardb);
        this.HCONF = this.get_hole_ranking(players, current_bettor_index);
        this.CALL_LEVEL = this.get_bet_level(this.CALL);
        this.BET_LEVEL = this.get_bet_level(this.genericMethods.current_bet); //feed      data we calc here so we don't gotta doubl do it!..
        this.POT_LEVEL = this.get_pot_level(players, current_bettor_index);
        this.BANKROLL = this.P.bankroll;
        var total_bankrolls = this.genericMethods.get_pot_size(players);
        for (var i = 0; i < players.length; i++) {
            total_bankrolls += players[i].bankroll;
            if (players[i].status != "BUST") {
                this.NUM_IN_GAME++;
                if (players[i].status != "FOLD") this.NUM_IN_HAND++;
            }
        }
        this.ID_CONF = "MID";
        ...
        this.SMALL = this.CALL + this.genericMethods.BIG_BLIND * 2; //consider MINIMUM RAISE here & below!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if (this.POT_LEVEL > 40) this.SMALL += 5;
        if (this.NUM_IN_GAME > 3) {
            this.MED = this.CALL + this.genericMethods.BIG_BLIND * 4;
            this.BIG = this.CALL + this.genericMethods.BIG_BLIND * 10;
        } else {
            this.SMALL += 5;
    ...
    get_hole_ranking(players, current_bettor_index) {
    ...
    get_pot_level(players, current_bettor_index) {
        var p = this.genericMethods.get_pot_size(players);
